...
async def cli(agent: str, session: str, history: bool):
    ...
    # To store the last user prompt for potential resending
    last_prompt = ""
    ...
    while True:
        ...
        if prompt.strip().lower() == ":-1":
            if not last_prompt:
                print("\n❌ No last message to resend.")
                return None
            else:
                print(f"\n🔄 Resending last message: {last_prompt}")
                prompt = last_prompt
        else:
            # Store the current prompt as last prompt for potential resending
            last_prompt = prompt
